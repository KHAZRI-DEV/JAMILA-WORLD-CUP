      ...
      <section data-testid="social-wall" className="py-24 bg-gradient-to-b from-black via-[#0f0509] to-black">
        <div className="max-w-[1600px] mx-auto px-6 md:px-12">
          <div className="flex items-end justify-between flex-wrap gap-6 mb-12">
            <div>
              <div className="text-xs tracking-[0.3em] text-[#D4AF37] uppercase font-bold mb-2">
                #MoroccoUSA2026
              </div>
              <h2
                className={`font-display font-black text-4xl md:text-6xl tracking-tighter ${
            ...
            <div className="flex items-center gap-3 text-white/60">
              <a
                ...
                className="p-3 border border-white/10 hover:border-[#D91C5C] hover:text-white transition-colors"
              >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(Array.isArray(posts) ? posts : []).map((p, i) => (
              <motion.div
                ...
                data-testid={`social-post-${p?.id || i}`}
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs text-white/50">
                  ...
                  <span className="flex items-center gap-1">
                    <Heart className="w-3 h-3 text-[#D91C5C]" />
                    {Number(p?.likes || 0).toLocaleString()}
                  </span>
                </div>
              </motion.div>
            ))}
      ...
      <section className="py-24 bg-black">
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 grid lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-5 space-y-6" data-testid="newsletter-block">
            <div className="text-xs tracking-[0.3em] text-[#D4AF37] uppercase font-bold">
              {t.footer.newsletter}
            </div>

            <h3
              className={`font-display font-black text-4xl md:text-5xl tracking-tighter leading-none ${
                locale === "ar" ? "font-arabic" : ""
              }`}
            >
              {locale === "ar"
                ? "ابق على اتصال"
                : locale === "en"
                ? "Stay close to the pride"
                : "Reste près de la fierté"}
            </h3>

            <p className="text-white/60 max-w-md">
              {locale === "ar"
                ? "احصل على الأخبار، عروض الإصدار المحدود، ومحتوى حصري."
                : locale === "en"
                ? "Get drops, news, and exclusive Atlas Lions content."
                : "Reçois les drops, news, et contenu exclusif des Lions de l'Atlas."}
            </p>

            <form
              onSubmit={submit}
              className="flex max-w-md border border-white/15 focus-within:border-[#D91C5C] transition-colors rounded-md"
            >
              <input
                type="email"
                required
          ...
          <div className="lg:col-span-7 space-y-6" data-testid="stores-block">
            <div className="flex items-end justify-between flex-wrap gap-4">
              <div>
                ...
                <h3
                  className={`font-display font-black text-4xl md:text-5xl tracking-tighter leading-none mt-2 ${
                    locale === "ar" ? "font-arabic" : ""
                  }`}
                >
                  Maroc · USA
            ...
            <div className="grid sm:grid-cols-2 gap-3 max-h-[420px] overflow-y-auto no-scrollbar pr-2">
              {(Array.isArray(stores) ? stores : []).map((s, i) => (
                <div
                  key={s?.id || i}
                  data-testid={`store-${s?.id || i}`}
                  className="border border-white/10 hover:border-[#D91C5C]/50 p-4 transition-colors group"
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div
                      className={`text-[10px] font-bold tracking-wider px-2 py-1 ${
                        s?.country === "MA"
                          ? "bg-[#D91C5C]/20 text-[#FF1F75]"
                          : "bg-[#006233]/20 text-[#00A050]"
                      } rounded-md`}
                    >
                      {s?.country || ""}
                    </div>
          ...
          {Array.from({ length: 2 }).map((_, i) => (
            <div key={i} className="flex items-center gap-12 px-6">
              {[
                ...
                <span
                  key={j}
                  className="font-display font-black text-3xl md:text-5xl text-white/10 tracking-tighter whitespace-nowrap"
                >
